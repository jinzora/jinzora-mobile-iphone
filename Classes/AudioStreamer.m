...
- (BOOL)openFileStream
{
	@synchronized(self)
	{
		...
		//
		NSLog(@"AudioStreamer::openFileStream - byte offset %d", self.offsetStart);

		CFHTTPMessageRef message= CFHTTPMessageCreateRequest(NULL, (CFStringRef)@"GET", (CFURLRef)url, kCFHTTPVersion1_1);
		if(self.offsetStart > 0) {
			CFStringRef byteOffset = CFStringCreateWithFormat(NULL, NULL, CFSTR("bytes=%d-"), self.offsetStart);
			CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Range"), byteOffset);
		} 
...
- (void)startWithOffsetInSecs:(UInt32) offsetInSecs
{
	...
	// 1 kilobit == 125 bytes
	int bitRateToUse = self.bitRate;
	if (bitRateToUse <1000) bitRateToUse*=1000;
	
	self.offsetStart = ( (bitRateToUse / 1000) * 125 * offsetInSecs ) + thisFileDataOffset;
	
	[self start];
	startTime = offsetInSecs;
}
...
- (double)progress
{
	@synchronized(self)
	{
		if (sampleRate > 0 && ![self isFinishing])
		{
			...
			double progress = startTime + seekTime + queueTime.mSampleTime / sampleRate;
			if (progress < 0.0)
			{
				progress = 0.0;
			}
...
/*			
			NSLog(@"Sample rate from property: %f  File sample rate: %f", asbd.mSampleRate, sampleRate);
...
///*
				self.errorCode = AS_AUDIO_QUEUE_CREATION_FAILED;
				if (err)
				{
					char *errChars = (char *)&err;
					NSLog(@"%@ err: %c%c%c%c %d\n",
						  [AudioStreamer stringForErrorCode:self.errorCode],
						  errChars[3], errChars[2], errChars[1], errChars[0],
						  (int)err);
					...
					errorCode = 0;
					[self 
					 performSelector:@selector(stop)
					 onThread:[NSThread mainThread]
					 withObject:nil
					 waitUntilDone:NO];
					[self 
					 performSelector:@selector(start)
					 onThread:[NSThread mainThread]
					 withObject:nil
					 waitUntilDone:NO];
					//[self start];
				...
				{
					NSLog(@"%@", [AudioStreamer stringForErrorCode:self.errorCode]);
					return;
				}
//*/
			}
...
//	NSLog(@"handleBufferCompleteForQueue - Queued buffers: %ld", buffersUsed);
//
